...
const destination  = (argv.demo) ? 'build/demo/' : (argv.pub) ? 'build/publish/' : 'build/development/';
...
const js = function() {
  return src( path.js )
    ...
    .pipe(gulpif(argv.demo, inject.append(`
    
    \n`)))
    .on('error', gutil.log)
    // .pipe(gulpif(argv.demo, uglify()))
    .pipe(gulpif(argv.demo, jobs({
      compact: true,
...
const img = function() {
  return src( path.img )
    .pipe(gulpif(argv.demo , imagemin([
      webp({
        quality: 75
      })
    ])))
...
const build = series(cleanit, html, scss, js, img, plugins, data, pwa, options, rtl);
exports.build = build;
